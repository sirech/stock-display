...
a.length;c++)g(a[c],false)}function m(a,c){a($.map(c,function(b){return{label:b.name+" ("+b.symbol+")"+(b.exchDisp?" in "+b.exchDisp:""),value:{id:b.symbol,name:b.name}}}))}function n(){$("#search_field").autocomplete({source:function(a,c){$.get("http://localhost:3023",{q:a.term},function(b){m(c,b)})},select:function(a,c){var b=c.item.value,d;a:{d=b.id;var e,h=loadStocks();for(e=0;e<h.length;e++)if(h[e].id===d){d=true;break a}d=false}d||g(b,true);$("#search_field").val(b.name);return false},focus:function(a,
c){var b=c.item.value;$("#search_field").val(b.name);return false},open:function(){$(this).removeClass("ui-corner-all").addClass("ui-corner-top")},close:function(){$(this).removeClass("ui-corner-top").addClass("ui-corner-all")}})}function o(){var a;a=loadStocks();return $.map(a,function(c){return escape(c.id)}).join(",")}function p(){$("#export").button({label:chrome.i18n.getMessage("export"),icons:{primary:"ui-icon-gear"}}).click(function(){var a;a=o();a=chrome.extension.getURL("import.html")+"?stocks="+
a;$("#export_url").val(a);return false})}var k={template:stockConfigTemplate()};jQuery(function(){$("#stocks ul").sortable({handle:".handle",update:function(){f()}}).disableSelection();document.title=chrome.i18n.getMessage("options_title",[chrome.i18n.getMessage("extension_name")]);$("#title div #give_title").text(chrome.i18n.getMessage("give_title"));$("#stocks_header div #start").text(chrome.i18n.getMessage("start"));$("#stocks_header div #search_field").val(chrome.i18n.getMessage("example",["Apple"]));
$("#bookmark #help_export").text(chrome.i18n.getMessage("help_export"));$("#usage #help_start").text(chrome.i18n.getMessage("help_start"));$("#usage #help_comp").text(chrome.i18n.getMessage("help_comp"));$("#usage #help_drag").text(chrome.i18n.getMessage("help_drag"));i();l();n();p()})})();
