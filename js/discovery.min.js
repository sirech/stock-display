...
function addStock(a,c){var b=config.template.clone();b.find(".stock_name").text(a.name);b.find(".stock_id").text(a.id);b.find(".delete").click(function(){removeStock($(this).parent());return false});$("#stocks ul").append(b);b.fadeIn("slow");c&&persist()}function unpersist(){var a,c;a=loadStocks();for(c=0;c<a.length;c++)addStock(a[c],false)}function showResults(a,c){a($.map(c,function(b){return{label:b.name+" ("+b.symbol+")"+(b.exchDisp?" in "+b.exchDisp:""),value:{id:b.symbol,name:b.name}}}))}
