...
max:"my"}};jQuery(function(){$("#status").ajaxStart(function(){$(this).show()});$("#status").ajaxError(function(){$(this).hide()});$("#status").ajaxStop(function(){$(this).fadeOut(2E3)});document.title=chrome.i18n.getMessage("options_title",[chrome.i18n.getMessage("extension_name")]);$("#title").click(function(){window.open(chrome.extension.getURL("standalone.html"));window.close();return false}).mouseenter(function(){$(this).addClass("title_highlight");return false}).mouseleave(function(){$(this).removeClass("title_highlight");
return false}).attr("title",chrome.i18n.getMessage("help_standalone")).text(getTitle());$("#disclaimer").text(chrome.i18n.getMessage("disclaimer"));$("#empty a").text(chrome.i18n.getMessage("empty_add"));$("#time_options li").addClass("ui-state-default").click(function(){store("time",$(this).attr("id"));h($("#chart").data("id"));return false});if(q()){i(c.stocks);window.setInterval(function(){i(c.stocks)},45E3)}})})();
