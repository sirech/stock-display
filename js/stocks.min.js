var app={modifiers:{name:"n",symbol:"s",last_price:"l1",last_time:"t1",percent_change:"p2",change:"c1"},format:{},stocks:loadStocks(),template:stockTemplate(),chartArguments:{today:"b",lastWeek:"w",threeMonths:"3m",sixMonths:"6m",lastYear:"1y",twoYears:"2y",fiveYears:"5y",max:"my"}};
function buildModifiers(){app.format={last_time:0,name:1,last_price:2,percent_change:3,change:4,symbol:5};return app.modifiers.last_time+app.modifiers.name+app.modifiers.last_price+app.modifiers.percent_change+app.modifiers.change+app.modifiers.symbol}function buildRequest(b){var a,d;a=[];for(d=0;d<b.length;d++)a.push(b[d].id);return"http://download.finance.yahoo.com/d/quotes.csv?s="+a.join("+")+"&f="+buildModifiers()}function sendRequest(b){$.get(b,function(a){present(a)})}
function refresh(b){app.stocks.length!==0&&sendRequest(buildRequest(b))}function buildChartRequest(b,a){var d,c="http://ichart.finance.yahoo.com/";if(a===app.chartArguments.today||a===app.chartArguments.lastWeek){c+=a+"?s=";d="&"}else{c+="c/"+a+"/";d="?"}return c+b+d+Math.floor(Math.random()*1E6)}
function showChart(b,a){var d,c;d=app.chartArguments[load("time")]||app.chartArguments.today;$("#chart").data("id",b);$("#chart ul").find("#"+load("time")).addClass("ui-state-active").siblings().removeClass("ui-state-active");c=new Image;c.src=buildChartRequest(b,d);c.onload=function(){$("#chart img").remove();a?$("#chart").append(c):$("#chart").append(c).effect("slide")}}
...
$("#disclaimer").text(chrome.i18n.getMessage("disclaimer"));$("#empty a").text(chrome.i18n.getMessage("empty_add"));$("#time_options li").addClass("ui-state-default").click(function(){store("time",$(this).attr("id"));showChart($("#chart").data("id"));return false});if(prepareList()){refresh(app.stocks);window.setInterval(function(){refresh(app.stocks)},45E3)}});
