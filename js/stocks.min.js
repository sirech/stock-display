...
function showChart(b){var a,c;a=app.chartArguments[localStorage.time]||app.chartArguments.today;$("#chart").data("id",b);$("#chart ul").find("#"+localStorage.time).addClass("ui-state-active").siblings().removeClass("ui-state-active");c=new Image;c.src=buildChartRequest(b,a);c.onload=function(){$("#chart img").remove();$("#chart").append(c).effect("slide")}}
...
function showEmptyMsg(){$("#empty").show().find("a").click(function(){window.open(chrome.extension.getURL("options.html"));window.close();return false});$("#title").hide();$("#disclaimer").hide();$("#last_update").hide()}function togglePrice(){toggleKey($("#stocks"),"price_chg");$("#stocks ul .stock .stock_chg").each(function(b,a){swap($(a),"alt")})}
