...
function present(b){var a,d,c,e,f;a=csvToArray(b);d=0;c=(b=getBoolKey($("#stocks"),"price_chg"))?app.format.change:app.format.percent_change;e=b?app.format.percent_change:app.format.change;$("#stocks ul .stock").each(function(i,h){var g=a[d];f=g[c].charAt(0)==="+"?"up":g[c].charAt(0)==="-"?"down":"neutral";$(h).find(".stock_price").text(g[app.format.last_price]);$(h).find(".stock_chg").removeClass("up down neutral").addClass(f).data("alt",g[e]).text(g[c]);d++});$("#chart img").length!=0&&showChart($("#chart").data("id"),
true)}function showEmptyMsg(){$("#empty").show().find("a").click(function(){window.open(chrome.extension.getURL("options.html"));window.close();return false});$("#title").hide();$("#disclaimer").hide();$("#last_update").hide()}function togglePrice(){toggleKey($("#stocks"),"price_chg");$("#stocks ul .stock .stock_chg").each(function(b,a){swap($(a),"alt")})}
function yahooTimeToDate(b){var a,d;a=b.slice(-2).toLowerCase();d=b.slice(0,-2).split(":");b=new Date;b.setHours(function(c,e){return e?c%12+12:c%12}(parseInt(d[0],10),a==="pm"));b.setMinutes(parseInt(d[1],10));b.setSeconds(0);a=240-b.getTimezoneOffset();a=b.getTime()+a*60*1E3;return new Date(a)}
